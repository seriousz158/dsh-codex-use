        ...
        function CodexRateLimitRow() {
          ...
          const refresh = react.useCallback(async (force = false) => {
            ...
            try {
              ...
              setState({ loading: false, status: status.ok ? status.value : null, limits: limits.ok ? limits.value : null, error: status.ok && limits.ok ? null : (status.error?.code ?? limits.error?.code ?? "rate-limits-unavailable") });
